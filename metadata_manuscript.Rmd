...
### Note ##
# This Template is not included in the finalt Metadata Template but accounted for in the analysis.
...
<!-- ![Fig 1. A schematic diagram of the metadata compilation process. From the original repository, three different datasets are represented: the first data type contains one topic: “landings”, the second contains two topics: “landings” and “revenue”, and the third contains  three topics: “landings, “aquaculture”, and “totals”. In addition, each dataset has multiple spatial components. The last column shows how the records would appear in the metadatabase.](Images/Methods_Flow.png) -->
...
<!-- ![Fig 2. Locations where metadata workshops were held and contributing institutions. Abbreviations in S4 Table.](Images/Effort_Map.png) -->
...
```{r International_Sour, eval=T, echo=F}
...
#        height = 6,
#        units = "in",
#        path = "/Users/carmelia/Dropbox/Metadata_Mexico/Manuscript/Submission/Figures")
...
<!-- ![Fig 4. Yearly metadata records by major research category. Results shown from year 1950 onward. See Fig 1-B for categories included within Other)](Images/Time_Plot.png) -->
...
ggsave("Time_Plot.png",
       ...
       height = 6,
       units = "in",
       path = "/Users/carmelia/Dropbox/Metadata_Mexico/Manuscript/Images")
...
# Not run when knitteing, it will take ages ##
...
<!-- ![Fig 5. Geographic localization of metadata according to sub-regions and research category. All values are in percentage except those that say “Record”; numbers within regions may not add to 100% due to exclusion of “other” types of research. Icons from Freepik downloaded from https://www.flaticon.com on 07/12/2018. Creative Commons BY 3.0](Images/Spatial_Results.png) -->
...
<!-- ![Fig 6. Characterization of institutions that host data, research field, and social-ecological interaction indicators. Thickness of grey connections represents the number of metadata records.](Images/Flow_Chart.png) -->
...
**S3 Table. List of authors and institutions represented in the metadatabase.** Note that not all authors were directly contacted; however, appropriated authorship was included as data were found publicly available in online-open-access repositories.

```{r S3_Table, eval=F,echo=F}

Autores <- Meta_Template %>% 
  group_by(Author) %>% 
  summarise(n()) # 284 authores

Instituciones <- Meta_Template %>% 
  group_by(Institution) %>% 
  summarise(n()) # 59 instituciones
